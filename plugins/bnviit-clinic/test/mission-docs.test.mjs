...
test('§7.2.3 maskFields fail-closed: 필드에 표지어+부분숫자·비정형 주소 잔존 시 residualRisk', async () => {
  const { maskFields } = await import(join(FAQ_DIR, 'preprocess.mjs'));

  // (a) content에 표지어("연락처")+부분숫자(끝자리)만 → 비정형 PII 잔존 → residualRisk.
  const a = maskFields({
    content: '연락처 끝자리 5678 로 회신주세요',
    source: 'knowledge/care.md',
    heading: '안내',
  });
  assert.equal(a.residualRisk, true, '표지어+부분숫자 잔존 시 residualRisk true');
  assert.ok(a.residualFields.includes('content'), '잔존 필드 보고(content)');
  assert.ok(!a.masked.content.includes('5678'), '잔존 위험 필드는 부분 원문 미잔류(차폐)');

  // (b) heading에 비정형 주소(행정구역+번지 변칙) 잔존 → residualRisk.
  const b = maskFields({
    content: '일반 안내',
    source: 'knowledge/케이스.md',
    heading: '주소 무슨동 12로 인근 환자',
  });
  assert.equal(b.residualRisk, true, '비정형 주소 잔존 시 residualRisk true');
  assert.ok(b.residualFields.includes('heading'), '잔존 필드 보고(heading)');

  // (c) 깨끗이 마스킹되는 명백 PII는 residualRisk false(과잉 차단 방지).
  const c = maskFields({
    content: '환자 010-1234-5678 문의',
    source: 'knowledge/care.md',
    heading: '회복 기간',
  });
  assert.equal(c.residualRisk, false, '명백 PII 정상 마스킹 시 residualRisk false');
  assert.ok(c.masked.content.includes('[전화번호]'), 'content 전화 마스킹');
});
