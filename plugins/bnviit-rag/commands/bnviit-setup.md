...
4. **② 도구 준비**: `${CLAUDE_PLUGIN_ROOT}/skills/bnviit-memory/rag/`를 직접 실행한다(`${CLAUDE_PLUGIN_ROOT}`는 Claude Code가 플러그인 루트 절대경로로 확장하는 공식 변수).
5. **③ 의존성**: `npm ci`(lockfile 기준)를 `"${CLAUDE_PLUGIN_ROOT}/skills/bnviit-memory/rag"` 디렉터리에서 실행한다. 큰 다운로드(모델 약 100MB)를 사전 고지.
...
10. **⑧ 완료 보고**: `node "${CLAUDE_PLUGIN_ROOT}/skills/bnviit-memory/rag/status.mjs" --root <프로젝트_루트>` 결과와 query 사용법을 안내.
